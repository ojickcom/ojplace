...
class JawanStart extends ConsumerStatefulWidget {
  const JawanStart({super.key});

  @override
  ConsumerState<JawanStart> createState() => _jawanStartState();
}

class _jawanStartState extends ConsumerState<JawanStart> {
  int browserNumber = 1;
  String userEmail = '';
  final startJawanProvider = FutureProvider<List<KeywordModel>>((ref) async {
    return ref.read(blogViewModelProvider.notifier).keywordStart();
  });
  @override
  void initState() {
    super.initState();
...
// 팝업창 띄우기
    final blogs = ref.watch(startJawanProvider);
